...
* **Global Merged Rows (Rows 1–3, 12–14):** Data values are stored in and retrieved from **Column D** (the primary cell for the merged horizontal range `B:F`).
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **1** | System Status | `D1` | **Input (Read)** | String | Global system state (`🔴 Stop` / `🟢 Start`). If `Stop`, halt all executions. |
| **2** | Overall Budget ($) | `D2` | **Input (Read)** | Integer / Float | Maximum daily spending limit across all active platforms combined. |
| **3** | Today's Remaining Overall Budget | `D3` | **Output (Write)** | Float / Number | Calculated remaining global daily budget. Updated after job execution. |
...
| **10** | Platform Budget / Day | `B10:F10` | **Input (Read)** | Float / Null | Platform-specific daily budget cap. If **Blank/Null**, fallback to `Overall Budget` (`D2`). |
| **11** | Today's Remaining Platform Budget | `B11:F11` | **Output (Write)** | Float / Null | Platform-specific remaining budget. Updated post-run. |
| **12** | Last System Alert | `D12` | **Output (Write)** | Timestamp | Timestamp of last system alert (`YYYY-MM-DD HH:MM:SS`). |
| **13** | Last Reset Date | `D13` | **Output (Write)** | Timestamp | Timestamp when daily budgets were last reset (`YYYY-MM-DD HH:MM:SS`). |
| **14** | Status | `D14` | **Output (Write)** | String | System execution message or error status log. |
...
* **System Control:**
  * System Status: `D1`
  * Overall Budget: `D2`
  * Today's Remaining Overall Budget: `D3`
* **System Output Logs:**
  * Last System Alert: `D12`
  * Last Reset Date: `D13`
  * Global Status Log: `D14`
